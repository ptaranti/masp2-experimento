...
public class HandleTimeRateBehaviorLinear3 extends HandleTimeRateBehaviour {
	...
	void errorControl() {
		...
		if (super.currentStatistics.getMaxErrorInformed() >= PropertiesLoaderImpl.ERROR_TRIGGER_AGGRESSIVE_CORRECTION/2
				&& counterStrongCorrection == 0) {

			if (super.currentStatistics
					.getNumberOfErrorsGreatherThenTriggerUpperLimit() > 0)
				System.out
				.println("ATENCAO ---- ATENCAO----LIMITE MAX ULTRAPASSADO");

			newRate = simulationClockControl.getSimulationClockRate()
					* (1 - ((super.currentStatistics.getMaxErrorInformed() - PropertiesLoaderImpl.ERROR_TRIGGER_AGGRESSIVE_CORRECTION/2) / (super.currentStatistics
							.getMaxErrorInformed() * PropertiesLoaderImpl.STABLES_CYCLES_TO_OPTIMIZE)));
			/*
			 * newRate = simulationClockControl.getSimulationClockRate() /
			 * (super.currentStatistics.getMaxErrorInformed() -
			 * PropertiesLoaderImpl.ERROR_TRIGGER + 1); if (newRate <
			 * (optimizeSimulationTimeRate(Math .sqrt(this.stabilityCounter -
			 * 1)))) newRate = optimizeSimulationTimeRate(Math
		...
		if (super.currentStatistics.getMaxErrorInformed() < PropertiesLoaderImpl.ERROR_TRIGGER_AGGRESSIVE_CORRECTION/2
				&& super.currentStatistics
				.getMaxErrorInformed() >= PropertiesLoaderImpl.ERROR_TRIGGER/2
				&& flag) {
