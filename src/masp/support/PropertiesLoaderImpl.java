...
public class PropertiesLoaderImpl {
	...
	public static final int JADE_PORT = findJadePort();
	...
	public static final boolean IS_SPATIAL_SIMULATION = findIsSpatialSimulation();

	
	/**
	 * @return porta logica a ser usada pela plataforma JADE
	 */
	private static int findJadePort() {

		if (overallClassTest)
			overallClassTest(); // teste completo de todas propriedades caso nao
		// tenha sido executado

		int port = Integer.parseInt(PropertiesLoaderImpl.getValor("JADE_PORT"));
		// System.out.println(port);
		return port;
	}

	private static boolean findIsSpatialSimulation() {
		return Boolean.parseBoolean(PropertiesLoaderImpl
				.getValor("IS_SPATIAL_SIMULATION"));
	}
